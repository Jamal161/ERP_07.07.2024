        ...
        public string mUpdateTour(TourConfig obj)
        {
            ...
            using (SqlConnection gcnMain = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    // Construct the SQL update statement with parameterized queries
                    strSQL = "UPDATE HRS_EMP_TOUR SET ";
                    strSQL += "TOUR_NAME = @TourName, ";
                    strSQL += "TOUR_STATUS = @TourStatus ";
                    strSQL += "WHERE SERIAL_NO = @SerialNo";

                    cmdUpdate.CommandText = strSQL;

                    // Add parameters to the command
                    cmdUpdate.Parameters.AddWithValue("@TourName", obj.strTOUR_NAME);
                    cmdUpdate.Parameters.AddWithValue("@TourStatus", obj.strTOUR_STATUS);
                    cmdUpdate.Parameters.AddWithValue("@SerialNo", obj.intSERIAL_NO);
            ...
            using (SqlConnection gcnMain = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    foreach (var tourPlan in tourPlans)
                    {
                        ...
                        strSQL = strSQL + "'" + tourPlan.strSTATUS + "'";
                ...
                using (SqlCommand cmd = new SqlCommand(strSQL, gcnMain))
                {
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            ...
                            //tour.strEFFECTIVE_DATE = Convert.ToDateTime(dr["EFFECTIVE_DATE"]).ToString("dd-MM-yyyy");
                            ...
                            tour.strSTATUS = dr["STATUS"].ToString();
