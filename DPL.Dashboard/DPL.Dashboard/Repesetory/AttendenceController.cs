                          ...
                          GROUP BY T.TEAM_NAME) AS TB1 LEFT OUTER JOIN
                             (SELECT        T.TEAM_NAME, 'MPO' AS Role, 0 AS NOMPO, COUNT(DISTINCT V.MPO_CARD_NO) AS Present, 0 AS LEAVE, 0 AS ABSENT
                               FROM            HRS_TRANS_WORK_ATTENDANCE AS A INNER JOIN
                                                         ...
                                                         SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME
                               WHERE         ( A.ATTEN_DATEIN = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                               + ")GROUP BY T.TEAM_NAME UNION ALL  SELECT        T.TEAM_NAME, 'AH' AS Role, 0 AS NOMPO, COUNT(DISTINCT V.AREA) AS Present, 0 AS LEAVE, 0 AS ABSENT FROM            HRS_TRANS_WORK_ATTENDANCE AS A INNER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON A.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE         ( A.ATTEN_DATEIN = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                                + ")  GROUP BY T.TEAM_NAME UNION ALL SELECT        T.TEAM_NAME, 'DH' AS Role, 0 AS NOMPO, COUNT(DISTINCT V.DIVISION) AS Present, 0 AS LEAVE, 0 AS ABSENT FROM  HRS_TRANS_WORK_ATTENDANCE AS A INNER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON A.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE ( A.ATTEN_DATEIN = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                               + ")  GROUP BY T.TEAM_NAME  UNION ALL SELECT   T.TEAM_NAME, 'ZH' AS Role, 0 AS NOMPO, COUNT(DISTINCT V.ZONE) AS Present, 0 AS LEAVE, 0 AS ABSENT   FROM HRS_TRANS_WORK_ATTENDANCE AS A INNER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON A.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE  ( A.ATTEN_DATEIN = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
