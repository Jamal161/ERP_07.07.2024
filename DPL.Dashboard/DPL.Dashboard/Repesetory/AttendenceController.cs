...
namespace DPL.DASHBOARD.Repesetory
{
    public class AttendenceController : Controller
    {
        ...
        public ActionResult mGetKibriadataRolewises(kibria obj)
        {

            var allLedger = mGetKibriadataRolewise(obj);
            return Json(allLedger, JsonRequestBehavior.AllowGet);

        }


        public ActionResult mGetKibriadataGroupwises(kibria obj)
        {

            var allLedger = mGetKibriadataGroupwise(obj);
            return Json(allLedger, JsonRequestBehavior.AllowGet);

        }

        public ActionResult mGetKibriadataGroupzonewises(kibria obj)
        {

            var allLedger = mGetKibriadataGroupzonewise(obj);
            return Json(allLedger, JsonRequestBehavior.AllowGet);
...
SELECT        tb1.TEAM_NAME, tb1.NOMPO, ISNULL(tb2.Present, 0) AS Present, ISNULL(tb3.LEAVE, 0) AS LEAVE, tb1.NOMPO - (ISNULL(tb2.Present, 0) + ISNULL(tb3.LEAVE, 0)) AS ABSENT
...
WHERE        (L.LEAVE_DATE = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                                + ")GROUP BY T.TEAM_NAME) AS tb3 RIGHT OUTER JOIN (SELECT        T.TEAM_NAME, 0 AS NOMPO, COUNT(DISTINCT V.MPO_CARD_NO) AS Present, 0 AS LEAVE, 0 AS ABSENT FROM            HRS_TRANS_WORK_ATTENDANCE_NEW AS A INNER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON A.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE        ( A.ATTEN_DATEIN = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                               + ") and A.ACTION='Approve' GROUP BY T.TEAM_NAME) AS tb2 ON tb3.TEAM_NAME = tb2.TEAM_NAME RIGHT OUTER JOIN (SELECT        T.TEAM_NAME, COUNT(DISTINCT V.MPO_CARD_NO) AS NOMPO, 0 AS Present, 0 AS LEAVE, 0 AS ABSENT FROM            SMART0005.dbo.ACC_LEDGER_Z_D_A AS V LEFT OUTER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME LEFT OUTER JOIN HRS_TRANS_WORK_ATTENDANCE_NEW AS A ON V.MPO_CARD_NO = A.EMP_CARD_NO WHERE        (V.LEDGER_STATUS = 0) AND (V.ZONE NOT IN ('ZH-Corporate Sales', 'X-MPO Accounts-Zone')) AND (V.TERRITORRY_NAME NOT IN ('Sample', 'Sales')) AND (V.BRANCH_ID IN ('0001', '0003')) AND  (V.LEDGER_STATUS = 0) GROUP BY T.TEAM_NAME) AS tb1 ON tb2.TEAM_NAME = tb1.TEAM_NAME;";
...
"'GROUP BY  V.ZONE , V.TERRITORRY_NAME) AS tb3 INNER JOIN (SELECT T.ZONE_NAME ,  V.TERRITORRY_NAME , 0 AS NOMPO, COUNT(DISTINCT V.MPO_CARD_NO) AS Present, 0 AS LEAVE, 0 AS ABSENT FROM HRS_TRANS_WORK_ATTENDANCE AS A INNER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON A.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME RIGHT OUTER JOIN SMART0005.dbo.MARKET_ROUTE AS R ON V.TERITORRY_CODE = R.TERITORRY_CODE AND (A.ATTEN_DATEIN = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
   + ")AND T.TEAM_NAME ='" + obj.TEAM_NAME + "' AND T.ZONE_NAME  ='" + obj.ZONE_NAME + "' AND V.MPO_DIV ='" + obj.MPO_DIV + "' AND V.MPO_AREA  ='" + obj.MPO_AREA + "' and v.TERRITORRY_NAME ='" + obj.TERRITORRY_NAME + "' GROUP BY  T.ZONE_NAME , V.TERRITORRY_NAME) AS tb2 ON tb3.ZONE = tb2.ZONE_NAME RIGHT OUTER JOIN (SELECT  T.ZONE_NAME ,   V.TERRITORRY_NAME , COUNT(DISTINCT V.MPO_CARD_NO) AS NOMPO, 0 AS Present, 0 AS LEAVE, 0 AS ABSENT FROM   SMART0005.dbo.ACC_LEDGER_Z_D_A AS V LEFT OUTER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME LEFT OUTER JOIN HRS_TRANS_WORK_ATTENDANCE AS A ON V.MPO_CARD_NO = A.EMP_CARD_NO RIGHT OUTER JOIN SMART0005.dbo.MARKET_ROUTE AS R ON V.TERITORRY_CODE = R.TERITORRY_CODE WHERE (V.LEDGER_STATUS = 0) AND (V.ZONE NOT IN ('ZH-Corporate Sales', 'X-MPO Accounts-Zone')) AND (V.TERRITORRY_NAME NOT IN ('Sample', 'Sales')) AND (V.BRANCH_ID IN ('0001', '0003')) AND  (V.LEDGER_STATUS = 0) AND T.TEAM_NAME ='" + obj.TEAM_NAME + "' AND T.ZONE_NAME  ='" + obj.ZONE_NAME + "' AND V.MPO_DIV ='" + obj.MPO_DIV + "' AND V.MPO_AREA  ='" + obj.MPO_AREA + "' and v.TERRITORRY_NAME ='" + obj.TERRITORRY_NAME + "' GROUP BY  T.ZONE_NAME , V.TERRITORRY_NAME) AS tb1 ON tb2.ZONE_NAME = tb1.ZONE_NAME";
        ...
        public List<kibria> mGetKibriadataRolewise(kibria obj)
        {
            string strSQL = null;
            string connectionString = Utility.SQLConnstringComSwitch("0001");
            ...
            using (SqlConnection gcnMain = new SqlConnection(connectionString))
            {
                ...
                strSQL = @"
                    SELECT        TB1.Position, TB1.Role, TB1.NOMPO, TB2.Present, tb3.LEAVE, TB1.NOMPO - (TB2.Present + tb3.LEAVE) AS ABSENT
                    FROM            (SELECT        1 AS Position, 'MPO' AS Role, COUNT(DISTINCT V.MPO_CARD_NO) AS NOMPO, 0 AS Present, 0 AS LEAVE, 0 AS ABSENT
                    FROM            SMART0005.dbo.ACC_LEDGER_Z_D_A AS V INNER JOIN
                    SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME
                    WHERE    
                    (V.LEDGER_STATUS = 0) AND (V.BRANCH_ID IN ('0001', '0003')) AND (V.TERRITORRY_NAME NOT IN ('Sample', 'Sales')) AND (V.ZONE NOT IN ('ZH-Corporate Sales', 'X-MPO Accounts-Zone'))
                    UNION ALL
                    SELECT        2 AS Position, 'AH' AS Role, COUNT(DISTINCT V.AREA) AS NOMPO, 0 AS Present, 0 AS LEAVE, 0 AS ABSENT
                    FROM            SMART0005.dbo.ACC_LEDGER_Z_D_A AS V INNER JOIN
                    SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME
                    WHERE        (V.LEDGER_STATUS = 0) AND (V.BRANCH_ID IN ('0001', '0003')) AND (V.TERRITORRY_NAME NOT IN ('Sample', 'Sales')) AND (V.ZONE NOT IN ('ZH-Corporate Sales', 'X-MPO Accounts-Zone'))
                    UNION ALL
                    SELECT        3 AS Position, 'DH' AS Role, COUNT(DISTINCT V.DIVISION) AS NOMPO, 0 AS Present, 0 AS LEAVE, 0 AS ABSENT
                    FROM            SMART0005.dbo.ACC_LEDGER_Z_D_A AS V INNER JOIN
                    SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME
                    WHERE        (V.LEDGER_STATUS = 0) AND (V.BRANCH_ID IN ('0001', '0003')) AND (V.TERRITORRY_NAME NOT IN ('Sample', 'Sales')) AND (V.ZONE NOT IN ('ZH-Corporate Sales', 'X-MPO Accounts-Zone'))
                    UNION ALL
                    SELECT        4 AS Position, 'ZH' AS Role, COUNT(DISTINCT V.ZONE) AS NOMPO, 0 AS Present, 0 AS LEAVE, 0 AS ABSENT
                    FROM            SMART0005.dbo.ACC_LEDGER_Z_D_A AS V INNER JOIN
                    SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME
                    WHERE        (V.LEDGER_STATUS = 0) AND (V.BRANCH_ID IN ('0001', '0003')) AND (V.TERRITORRY_NAME NOT IN ('Sample', 'Sales')) AND (V.ZONE NOT IN ('ZH-Corporate Sales', 'X-MPO Accounts-Zone'))) AS TB1 INNER JOIN
                    (SELECT        1 AS Position, 'MPO' AS Role, 0 AS NOMPO, COUNT(DISTINCT V.MPO_CARD_NO) AS Present, 0 AS LEAVE, 0 AS ABSENT
                    FROM            HRS_TRANS_WORK_ATTENDANCE AS A INNER JOIN
                    SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON A.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN
                    SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME
                    WHERE        ( A.ATTEN_DATEIN = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                    + ")UNION ALL SELECT  2 AS Position, 'AH' AS Role, 0 AS NOMPO, COUNT(DISTINCT V.AREA) AS Present, 0 AS LEAVE, 0 AS ABSENT FROM HRS_TRANS_WORK_ATTENDANCE AS A INNER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON A.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE ( A.ATTEN_DATEIN = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                    + ") UNION ALL SELECT  3 AS Position, 'DH' AS Role, 0 AS NOMPO, COUNT(DISTINCT V.DIVISION) AS Present, 0 AS LEAVE, 0 AS ABSENT FROM  HRS_TRANS_WORK_ATTENDANCE AS A INNER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON A.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE ( A.ATTEN_DATEIN = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                    + ") UNION ALL SELECT  4 AS Position, 'ZH' AS Role, 0 AS NOMPO, COUNT(DISTINCT V.ZONE) AS Present, 0 AS LEAVE, 0 AS ABSENT FROM  HRS_TRANS_WORK_ATTENDANCE AS A INNER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON A.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE  ( A.ATTEN_DATEIN = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy")) + ")) AS TB2 ON TB1.Position = TB2.Position AND TB1.Role = TB2.Role INNER JOIN (SELECT 1 AS Position, 'MPO' AS Role, 0 AS NOMPO, 0 AS Present, COUNT(DISTINCT L.EMP_CARD_NO) AS LEAVE, 0 AS ABSENT FROM  HRS_EMP_LEAVE_DETAILS AS L LEFT OUTER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON L.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE (L.LEAVE_DATE = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                    + ") UNION ALL SELECT  2 AS Position, 'AH' AS Role, 0 AS NOMPO, 0 AS Present, COUNT(DISTINCT V.AREA) AS LEAVE, 0 AS ABSENT FROM  HRS_EMP_LEAVE_DETAILS AS L LEFT OUTER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON L.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE  (L.LEAVE_DATE = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                    + ") UNION ALL SELECT 3 AS Position, 'DH' AS Role, 0 AS NOMPO, 0 AS Present, COUNT(DISTINCT V.DIVISION) AS LEAVE, 0 AS ABSENT FROM HRS_EMP_LEAVE_DETAILS AS L LEFT OUTER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON L.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE  (L.LEAVE_DATE = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                    + ") UNION ALL SELECT        4 AS Position, 'ZH' AS Role, 0 AS NOMPO, 0 AS Present, COUNT(DISTINCT V.ZONE) AS LEAVE, 0 AS ABSENT FROM HRS_EMP_LEAVE_DETAILS AS L LEFT OUTER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON L.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE  (L.LEAVE_DATE = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy")) + ")) AS tb3 ON TB1.Position = tb3.Position AND TB1.Role = tb3.Role;";
                ...
                while (dr.Read())
                {
                    ...
                    oLedg.Position = dr["Position"].ToString();
        ...
        public List<kibria> mGetKibriadataGroupwise(kibria obj)
        {
            string strSQL = null;
            string connectionString = Utility.SQLConnstringComSwitch("0001");
            ...
            using (SqlConnection gcnMain = new SqlConnection(connectionString))
            {
                ...
                strSQL = @"
 SELECT        TB1.TEAM_NAME, TB1.Role, TB1.NOMPO, isnull(TB2.Present,0)Present, isnull(tb3.LEAVE,0) as LEAVE, isnull( TB1.NOMPO - (TB2.Present + tb3.LEAVE),0) AS ABSENT
                          FROM            (SELECT        T.TEAM_NAME, 'MPO' AS Role, COUNT(DISTINCT V.MPO_CARD_NO) AS NOMPO, 0 AS Present, 0 AS LEAVE, 0 AS ABSENT
                          FROM            SMART0005.dbo.ACC_LEDGER_Z_D_A AS V LEFT OUTER JOIN
                                                    SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME
                          WHERE        (V.LEDGER_STATUS = 0) AND (V.BRANCH_ID IN ('0001', '0003')) AND (V.TERRITORRY_NAME NOT IN ('Sample', 'Sales')) AND (V.ZONE NOT IN ('ZH-Corporate Sales', 'X-MPO Accounts-Zone'))
                          GROUP BY T.TEAM_NAME
                          UNION ALL
                          SELECT        T.TEAM_NAME, 'AH' AS Role, COUNT(DISTINCT V.AREA) AS NOMPO, 0 AS Present, 0 AS LEAVE, 0 AS ABSENT
                          FROM            SMART0005.dbo.ACC_LEDGER_Z_D_A AS V INNER JOIN
                                                   SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME
                          WHERE        (V.LEDGER_STATUS = 0) AND (V.BRANCH_ID IN ('0001', '0003')) AND (V.TERRITORRY_NAME NOT IN ('Sample', 'Sales')) AND (V.ZONE NOT IN ('ZH-Corporate Sales', 'X-MPO Accounts-Zone'))
                          GROUP BY T.TEAM_NAME
                          UNION ALL
                          SELECT        T.TEAM_NAME, 'DH' AS Role, COUNT(DISTINCT V.DIVISION) AS NOMPO, 0 AS Present, 0 AS LEAVE, 0 AS ABSENT
                          FROM            SMART0005.dbo.ACC_LEDGER_Z_D_A AS V INNER JOIN
                                                   SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME
                          WHERE        (V.LEDGER_STATUS = 0) AND (V.BRANCH_ID IN ('0001', '0003')) AND (V.TERRITORRY_NAME NOT IN ('Sample', 'Sales')) AND (V.ZONE NOT IN ('ZH-Corporate Sales', 'X-MPO Accounts-Zone'))
                          GROUP BY T.TEAM_NAME
                          UNION ALL
                          SELECT        T.TEAM_NAME, 'ZH' AS Role, COUNT(DISTINCT V.ZONE) AS NOMPO, 0 AS Present, 0 AS LEAVE, 0 AS ABSENT
                          FROM            SMART0005.dbo.ACC_LEDGER_Z_D_A AS V INNER JOIN
                                                   SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME
                          WHERE        (V.LEDGER_STATUS = 0) AND (V.BRANCH_ID IN ('0001', '0003')) AND (V.TERRITORRY_NAME NOT IN ('Sample', 'Sales')) AND (V.ZONE NOT IN ('ZH-Corporate Sales', 'X-MPO Accounts-Zone'))
                          GROUP BY T.TEAM_NAME) AS TB1 LEFT OUTER JOIN
                             (SELECT        T.TEAM_NAME, 'MPO' AS Role, 0 AS NOMPO, COUNT(DISTINCT V.MPO_CARD_NO) AS Present, 0 AS LEAVE, 0 AS ABSENT
                               FROM            HRS_TRANS_WORK_ATTENDANCE_NEW AS A INNER JOIN
                                                         SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON A.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN
                                                         SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME
                               WHERE         ( A.ATTEN_DATEIN = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                               + ")GROUP BY T.TEAM_NAME UNION ALL  SELECT        T.TEAM_NAME, 'AH' AS Role, 0 AS NOMPO, COUNT(DISTINCT V.AREA) AS Present, 0 AS LEAVE, 0 AS ABSENT FROM            HRS_TRANS_WORK_ATTENDANCE_NEW AS A INNER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON A.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE         ( A.ATTEN_DATEIN = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                                + ")  GROUP BY T.TEAM_NAME UNION ALL SELECT        T.TEAM_NAME, 'DH' AS Role, 0 AS NOMPO, COUNT(DISTINCT V.DIVISION) AS Present, 0 AS LEAVE, 0 AS ABSENT FROM  HRS_TRANS_WORK_ATTENDANCE_NEW AS A INNER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON A.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE ( A.ATTEN_DATEIN = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                               + ")  GROUP BY T.TEAM_NAME  UNION ALL SELECT   T.TEAM_NAME, 'ZH' AS Role, 0 AS NOMPO, COUNT(DISTINCT V.ZONE) AS Present, 0 AS LEAVE, 0 AS ABSENT   FROM HRS_TRANS_WORK_ATTENDANCE_NEW AS A INNER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON A.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE  ( A.ATTEN_DATEIN = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                                 + ") AND (T.TEAM_NAME = 'CHALLENGER') GROUP BY T.TEAM_NAME) AS TB2 ON TB1.Role = TB2.Role AND TB1.TEAM_NAME = TB2.TEAM_NAME LEFT OUTER JOIN (SELECT        T.TEAM_NAME, 'MPO' AS Role, 0 AS NOMPO, 0 AS Present, COUNT(DISTINCT L.EMP_CARD_NO) AS LEAVE, 0 AS ABSENT FROM   HRS_EMP_LEAVE_DETAILS AS L LEFT OUTER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON L.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE (L.LEAVE_DATE = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                              + ") GROUP BY T.TEAM_NAME UNION ALL SELECT        T.TEAM_NAME, 'AH' AS Role, 0 AS NOMPO, 0 AS Present, COUNT(DISTINCT V.AREA) AS LEAVE, 0 AS ABSENT FROM HRS_EMP_LEAVE_DETAILS AS L LEFT OUTER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON L.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME   WHERE (L.LEAVE_DATE = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                                 + ") GROUP BY T.TEAM_NAME UNION ALL SELECT        T.TEAM_NAME, 'DH' AS Role, 0 AS NOMPO, 0 AS Present, COUNT(DISTINCT V.DIVISION) AS LEAVE, 0 AS ABSENT  FROM  HRS_EMP_LEAVE_DETAILS AS L LEFT OUTER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON L.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME   WHERE (L.LEAVE_DATE = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                                + ") GROUP BY T.TEAM_NAME  UNION ALL SELECT        T.TEAM_NAME, 'ZH' AS Role, 0 AS NOMPO, 0 AS Present, COUNT(DISTINCT V.ZONE) AS LEAVE, 0 AS ABSENT  FROM HRS_EMP_LEAVE_DETAILS AS L LEFT OUTER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON L.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE  (L.LEAVE_DATE = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                                + ") AND (T.TEAM_NAME = 'CHALLENGER')  GROUP BY T.TEAM_NAME) AS tb3 ON TB1.Role = tb3.Role AND TB1.TEAM_NAME = tb3.TEAM_NAME;";
        ...
        public List<kibria> mGetKibriadataGroupzonewise(kibria obj)
        {
            string strSQL = null;
            string connectionString = Utility.SQLConnstringComSwitch("0001");
            ...
            using (SqlConnection gcnMain = new SqlConnection(connectionString))
            {
                ...
                strSQL = @"
                                              SELECT        TB1.ZONE as ZONE_NAME, TB1.Role, TB1.NOMPO, ISNULL( TB2.Present,0)Present, isnull(tb3.LEAVE,0)as LEAVE, isnull(TB1.NOMPO - (TB2.Present + tb3.LEAVE),0) AS ABSENT
                          FROM            (SELECT        V.ZONE, 'MPO' AS Role, COUNT(DISTINCT V.MPO_CARD_NO) AS NOMPO, 0 AS Present, 0 AS LEAVE, 0 AS ABSENT
                          FROM            SMART0005.dbo.ACC_LEDGER_Z_D_A AS V INNER JOIN
                                                    SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME
                          WHERE        (V.LEDGER_STATUS = 0) AND (V.BRANCH_ID IN ('0001', '0003')) AND (V.TERRITORRY_NAME NOT IN ('Sample', 'Sales')) AND (V.ZONE NOT IN ('ZH-Corporate Sales', 'X-MPO Accounts-Zone')) AND 
                           (T.TEAM_NAME = '" + obj.TEAM_NAME + "') GROUP BY V.ZONE UNION ALL SELECT V.ZONE, 'AH' AS Role, COUNT(DISTINCT V.AREA) AS NOMPO, 0 AS Present, 0 AS LEAVE, 0 AS ABSENT FROM SMART0005.dbo.ACC_LEDGER_Z_D_A AS V INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME  WHERE        (V.LEDGER_STATUS = 0) AND (V.BRANCH_ID IN ('0001', '0003')) AND (V.TERRITORRY_NAME NOT IN ('Sample', 'Sales')) AND (V.ZONE NOT IN ('ZH-Corporate Sales', 'X-MPO Accounts-Zone')) AND  (T.TEAM_NAME = '" + obj.TEAM_NAME + "') GROUP BY V.ZONE  UNION ALL  SELECT        V.ZONE, 'DH' AS Role, COUNT(DISTINCT V.DIVISION) AS NOMPO, 0 AS Present, 0 AS LEAVE, 0 AS ABSENT     FROM            SMART0005.dbo.ACC_LEDGER_Z_D_A AS V INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE        (V.LEDGER_STATUS = 0) AND (V.BRANCH_ID IN ('0001', '0003')) AND (V.TERRITORRY_NAME NOT IN ('Sample', 'Sales')) AND (V.ZONE NOT IN ('ZH-Corporate Sales', 'X-MPO Accounts-Zone')) AND (T.TEAM_NAME = '" + obj.TEAM_NAME + "') GROUP BY V.ZONE UNION ALL SELECT        V.ZONE, 'ZH' AS Role, COUNT(DISTINCT V.ZONE) AS NOMPO, 0 AS Present, 0 AS LEAVE, 0 AS ABSENT FROM            SMART0005.dbo.ACC_LEDGER_Z_D_A AS V INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE        (V.LEDGER_STATUS = 0) AND (V.BRANCH_ID IN ('0001', '0003')) AND (V.TERRITORRY_NAME NOT IN ('Sample', 'Sales')) AND (V.ZONE NOT IN ('ZH-Corporate Sales', 'X-MPO Accounts-Zone')) AND (T.TEAM_NAME = '" + obj.TEAM_NAME  + "') GROUP BY V.ZONE) AS TB1 left JOIN (SELECT        V.ZONE, 'MPO' AS Role, 0 AS NOMPO, COUNT(DISTINCT V.MPO_CARD_NO) AS Present, 0 AS LEAVE, 0 AS ABSENT FROM HRS_TRANS_WORK_ATTENDANCE AS A INNER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON A.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE        ( A.ATTEN_DATEIN = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy")) + 
                           ") AND (T.TEAM_NAME = '" + obj.TEAM_NAME + "') GROUP BY V.ZONE UNION ALL SELECT        V.ZONE, 'AH' AS Role, 0 AS NOMPO, COUNT(DISTINCT V.AREA) AS Present, 0 AS LEAVE, 0 AS ABSENT  FROM            HRS_TRANS_WORK_ATTENDANCE AS A INNER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON A.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE        ( A.ATTEN_DATEIN = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                               + ") AND (T.TEAM_NAME = '" + obj.TEAM_NAME + "') GROUP BY V.ZONE UNION ALL SELECT        V.ZONE, 'DH' AS Role, 0 AS NOMPO, COUNT(DISTINCT V.DIVISION) AS Present, 0 AS LEAVE, 0 AS ABSENT FROM            HRS_TRANS_WORK_ATTENDANCE AS A INNER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON A.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE        ( A.ATTEN_DATEIN = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                               + ") AND (T.TEAM_NAME = '" + obj.TEAM_NAME + "') GROUP BY V.ZONE  UNION ALL SELECT        V.ZONE, 'ZH' AS Role, 0 AS NOMPO, COUNT(DISTINCT V.ZONE) AS Present, 0 AS LEAVE, 0 AS ABSENT  FROM            HRS_TRANS_WORK_ATTENDANCE AS A INNER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON A.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE        ( A.ATTEN_DATEIN = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                             + ") AND (T.TEAM_NAME = '" + obj.TEAM_NAME + "') GROUP BY V.ZONE) AS TB2 ON TB1.Role = TB2.Role AND TB1.ZONE = TB2.ZONE left JOIN (SELECT        V.ZONE, 'MPO' AS Role, 0 AS NOMPO, 0 AS Present, COUNT(DISTINCT L.EMP_CARD_NO) AS LEAVE, 0 AS ABSENT FROM            HRS_EMP_LEAVE_DETAILS AS L LEFT OUTER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON L.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE        (L.LEAVE_DATE = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                             + ") AND (T.TEAM_NAME = '" + obj.TEAM_NAME + "') GROUP BY V.ZONE UNION ALL SELECT        V.ZONE, 'AH' AS Role, 0 AS NOMPO, 0 AS Present, COUNT(DISTINCT V.AREA) AS LEAVE, 0 AS ABSENT FROM            HRS_EMP_LEAVE_DETAILS AS L LEFT OUTER JOIN  SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON L.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME  WHERE        (L.LEAVE_DATE = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                              + ") AND (T.TEAM_NAME = '" + obj.TEAM_NAME + "') GROUP BY V.ZONE UNION ALL  SELECT        V.ZONE, 'DH' AS Role, 0 AS NOMPO, 0 AS Present, COUNT(DISTINCT V.DIVISION) AS LEAVE, 0 AS ABSENT   FROM            HRS_EMP_LEAVE_DETAILS AS L LEFT OUTER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON L.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN  SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE        (L.LEAVE_DATE = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                              + ") AND (T.TEAM_NAME = '" + obj.TEAM_NAME + "') GROUP BY V.ZONE UNION ALL SELECT        V.ZONE, 'ZH' AS Role, 0 AS NOMPO, 0 AS Present, COUNT(DISTINCT V.ZONE) AS LEAVE, 0 AS ABSENT FROM            HRS_EMP_LEAVE_DETAILS AS L LEFT OUTER JOIN SMART0005.dbo.ACC_LEDGER_Z_D_A AS V ON L.EMP_CARD_NO = V.MPO_CARD_NO INNER JOIN SMART0005.dbo.TEAM_CONFIG AS T ON V.ZONE = T.ZONE_NAME WHERE        (L.LEAVE_DATE = " + Utility.cvtSQLDateString(DateTime.Now.ToString("dd-MM-yyyy"))
                              + ") AND (T.TEAM_NAME = '" + obj.TEAM_NAME + "') GROUP BY V.ZONE) AS tb3 ON TB1.Role = tb3.Role AND TB1.ZONE = tb3.ZONE;";
